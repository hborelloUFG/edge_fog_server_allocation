...
def get_topology(net_name, cluster=6, capacity_percent=1.0, folder='../..'):
    df_nodes = pd.read_csv(folder+'/data/topologies/'+net_name+'_nodes_capacity.csv')
    df_edges = pd.read_csv(folder+'/data/topologies/'+net_name+'_edges_latency.csv')
...
def get_application(app_name, folder='../..'):
    # Read JSON data from file
    
    with open(folder+'/data/applications/'+app_name+'.json', 'r') as json_file:
...
# Normalization of network topology data and services
# Nodes and Services must be passed with the following data: 
#   - cpu, memory, storage and bandwidth
#   - numpy
def normalize_data(node, services, feature_range=((1, 10))):
    node_to = np.array(node)
    services_to = np.array(services)
    for i in range(1, 5):
        data = (np.concatenate((node_to[:,i], services_to[:,i]))) 
        data = data.reshape(-1, 1)

        data_norm = pre.MinMaxScaler(feature_range=feature_range).fit_transform(data)

        nodes_norm = (data_norm[:len(node_to)])
        services_norm = (data_norm[len(node_to):])
        node_to[:, i] = nodes_norm[:, 0]
        services_to[:, i] = services_norm[:, 0]

    return node_to, services_to
